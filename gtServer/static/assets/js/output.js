...
if (params.get("type")=="international"){
    fetch("http://localhost:8080/api/international" + search)
    .then (async response => {
        if (!response.ok) {
            throw new Error(`Request failed with status ${reponse.status}`)
        }
...
if (params.get("type")=="sms"){
    fetch("http://localhost:8080/api/sms" + search)
    .then (async response => {
        if (!response.ok) {
            throw new Error(`Request failed with status ${reponse.status}`)
        }
...
if (params.get("type")=="graph"){
    ...
    .then(data => {
        ...
        window.open("../site/graph_demo.html", "_blank");
    })
    .catch(error => console.log(error))
}
...
if (params.get("type")=="socs"){
    fetch("http://localhost:8080/api/socs" + search)
    .then (async response => {
        if (!response.ok) {
            throw new Error(`Request failed with status ${reponse.status}`)
        }
